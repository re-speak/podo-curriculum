...
- **The student app removes the teaching switch; absence of a role does not.** Lemonboard injects `PODO_LESSON_CONTEXT.viewerRole = "student"` only when podo-web explicitly marks the screen as a student view. `pager.js` removes the switch for that exact value. A missing context is an internal share or standalone review, so it keeps the switch visible by default.

## The shared pointer
- **The lesson is spoken over two screens, so the tutor needs a finger.** Tapping a block rings it in red on *both* screens, and exactly one thing is ever lit. It is symmetric — the learner points too. Red is the pointer's alone: a colour that also means something else stops meaning "here". What travels between the screens is *which block is lit*, never the click — see [`interactive/interaction-protocol.md`](./interactive/interaction-protocol.md).
