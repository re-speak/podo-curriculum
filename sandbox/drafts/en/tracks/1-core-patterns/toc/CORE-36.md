# CORE-36 · I was cooking when you called

> **Generated by `tools/authoring/en/build_lesson_briefs.py` — do not edit this file.**
>
> The track TOC is authoritative. Fix it or the parser, then regenerate this brief.

## Identity

- **Track:** `1-core-patterns`
- **Stable review id:** `CORE-36`
- **Level:** A2

## Outcome

- **Can do:** say what was in progress when something else happened.
- **Unit:** 6 · Tell what happened

## Required production

### Pattern 1

- **Model:** I was cooking when you called.
- **Frame:** `I was ___ing when you ___ed.`

### Pattern 2

- **Model:** While I was cooking, he set the table.
- **Frame:** `While I was ___ing, he ___.`

## Supporting content

- **Expressions:** `all of a sudden` · `at that moment`
- **Grammar:** the long thing takes `was ___ing`; the thing that interrupts takes the plain past.
- **Japanese-L1 risk:** `〜ていた` maps onto this cleanly, so it is one of the easier past lessons — the difficulty is holding two clauses, not the aspect.

## Planned spiral review

- **ASPECT-01 · contrast:** distinguish routine/state from action in progress
- **SUBJ-01 · transfer:** keep an explicit English subject

## Sequence guardrails

- **Already learned:** Core 1–35.
- **Not yet:** Core 37–122. Do not use their forms in learner-produced language.
- **Most recent available forms:** CORE-33 Did you have a good time? · CORE-34 I didn't sleep well · CORE-35 First we ate, then we walked around
- **Nearest protected forms:** CORE-37 I'm going to look for a new job · CORE-38 Let me help · CORE-39 I might be late
- The readiness gate applies to the complete learner-produced model, including every fixed word.
- Apart from this lesson's two taught patterns, productive grammar must come from earlier Core lessons.
- At Pre-A1/A1, at most one explicitly declared bounded survival chunk may run ahead; keep it whole and do not transform or assess its internal grammar.
- Tutor and partner English may run ahead only as supported receptive language; it must not become an unlabelled learner target.
- Spiral review reuses the current can-do and does not add a third pattern or a second grammar explanation.

## Neighbouring items

- **Previous:** CORE-35 · First we ate, then we walked around
- **Next:** CORE-37 · I'm going to look for a new job

## Authoring inputs — read in order

1. `../../../../ux-philosophy.md`
2. `../../../AGENTS.md`
3. `../../../LESSON-CREATION-WORKFLOW.md`
4. `../lesson-blueprint.md` (must exist before this track's pilot)
5. This brief
6. The complete canonical deck named by the blueprint
