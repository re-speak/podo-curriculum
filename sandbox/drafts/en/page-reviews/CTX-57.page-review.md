...
- Visible target or prompt: Lead through genuine uncertainty: Separate confirmed facts from open questions and give a specific update time. / 確認済みの事実と未確認の点を分け、次の更新時刻を示しましょう。
- Learning target: Orient the learner to the exact lead through genuine uncertainty outcome before role-play.
...
- Tutor action: Play the Executive and begin with the first counterpart line.
- Visible target or prompt: Giving a factual update during an investigation; learner role is the course role and tutor role is Executive (経営責任者); first tutor line: “Is customer data at risk?”
...
- Component consistency: Uses the shared six-turn dialogue with a stable learner/Executive role pair.
- Extracted evidence: `{"targetHighlights": [], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Learner action: Listen to each tutor-owned Executive line and choose its matching meaning.
- Tutor action: Read only each complete English Executive line; do not read the answer choices.
- Visible target or prompt: Tutor-owned Executive lines: “Is customer data at risk?”; “Do we know what caused the incident?”; “Good. I'll direct questions to the four o'clock update.”; “When will we know the full impact?”
- Learning target: Build receptive control of four complete Executive lines needed to understand the business task.
...
- Visible target or prompt: confirmed fact (確認済みの事実); open question (未解決の疑問)
...
- Extracted evidence: `{"targetHighlights": ["What we know is that", "現時点で分かっているのは", "What we know is that", "現時点で分かっているのは", "What we know is that", "現時点で分かっているのは"], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Extracted evidence: `{"targetHighlights": ["What we know is that", "現時点で分かっているのは", "What we know is that", "現時点で分かっているのは", "What we know is that", "現時点で分かっているのは", "What we know is that", "現時点で分かっているのは"], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Extracted evidence: `{"targetHighlights": ["現時点で分かっているのは", "現時点で分かっているのは", "現時点で分かっているのは", "現時点で分かっているのは"], "blankAnswers": ["What we know is that", "What we know is that", "What we know is that", "What we know is that"], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Extracted evidence: `{"targetHighlights": [], "blankAnswers": ["What we know is that customer data remains secure.", "What we know is that the backup is intact.", "What we know is that payments are still processing.", "What we know is that only one region is affected."], "hintChips": ["顧客データ:customer data", "バックアップ:backup", "決済処理:payments", "一つの地域:one region"], "supportStage": "supported", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Extracted evidence: `{"targetHighlights": [], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": ["Use “What we know is that ___” to separate a verified fact from what is still uncertain."]}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Choice/distractor quality: The examples vary the business content instead of presenting distractors.
- Component consistency: Uses the shared pattern teaching component with one hero and supporting examples.
- Extracted evidence: `{"targetHighlights": ["We don't know", "but we'll", "分かりません", "ただし、必ず", "We don't know", "but we'll", "分かりません", "ただし、必ず", "We don't know", "but we'll", "分かりません", "ただし、必ず"], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Choice/distractor quality: Four varied models provide practice without a redundant choice task.
- Component consistency: Uses the shared four-line read-aloud list.
- Extracted evidence: `{"targetHighlights": ["We don't know", "but we'll", "分かりません", "ただし、必ず", "We don't know", "but we'll", "分かりません", "ただし、必ず", "We don't know", "but we'll", "分かりません", "ただし、必ず", "We don't know", "but we'll", "分かりません", "ただし、必ず"], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Choice/distractor quality: No distractors are needed; each blank maps one-to-one to a highlighted target cue.
- Component consistency: Uses the shared target-only slot-input component across four rows.
- Extracted evidence: `{"targetHighlights": ["分かりません", "ただし、必ず", "分かりません", "ただし、必ず", "分かりません", "ただし、必ず", "分かりません", "ただし、必ず"], "blankAnswers": ["We don't know", "but we'll", "We don't know", "but we'll", "We don't know", "but we'll", "We don't know", "but we'll"], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Choice/distractor quality: Open production avoids an obvious two-choice answer and requires a complete sentence.
- Component consistency: Uses the shared target-v2 translation component with one declared support stage.
- Extracted evidence: `{"targetHighlights": [], "blankAnswers": ["We don't know that yet, but we'll update everyone at four.", "We don't know the cause yet, but we'll update everyone at six.", "We don't know the full impact yet, but we'll share another update at noon.", "We don't know when service will resume yet, but we'll update this page hourly."], "hintChips": ["4時:four", "原因:cause", "6時:six", "影響の全容:full impact", "正午:noon", "サービスの再開時刻:service resumption", "1時間ごと:hourly"], "supportStage": "supported", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Choice/distractor quality: Open production is appropriate because the learner creates one relevant sentence.
- Component consistency: Uses the shared Student's sentence and Correction capture fields.
- Extracted evidence: `{"targetHighlights": [], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": ["Use “We don't know ___ yet, but we'll ___” to admit one unknown and promise a specific update."]}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Visible target or prompt: Full-scene practice combines “What we know is that customer data remains secure.” and “We don't know that yet, but we'll update everyone at four.”.
...
- Extracted evidence: `{"targetHighlights": ["What we know is that", "現時点で分かっているのは", "We don't know", "but we'll", "分かりません", "ただし、必ず"], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Extracted evidence: `{"targetHighlights": [], "blankAnswers": ["What we know is that", "We don't know", "but we'll"], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": []}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Extracted evidence: `{"targetHighlights": [], "blankAnswers": [], "hintChips": [], "supportStage": "not-applicable", "substantivePrompts": ["During uncertainty, what helps you trust a leader's update?", "What about you—what makes an update trustworthy?"]}`
- Visual QA: 360px **pending**; 480px **pending**
- Verdict: **pass**
...
- Visible target or prompt: 'Confirmed' is stronger than 'likely': Use 'confirmed' only for verified information. 'Likely' still describes a probability, not a fact. Examples: the backup is confirmed intact / the backup is likely intact
