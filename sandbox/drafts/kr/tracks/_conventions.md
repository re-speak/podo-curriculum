...
| Needed | State | Why |
| --- | --- | --- |
| **the type scale came down** | **written · changes every Korean deck** | `.known.lines .k` **22px → 18px** and `.br-cn`/`.br-ko` **33px → 26px**. Raised from the English side, but it was never an English problem: 22px is exactly `.section-title`, so a `lesson-goal` stack read as a page of headlines rather than as the exchange, and a Korean line only ever needed 208px of its 342px row. Nothing wrapped before and nothing wraps now — Korean pages just get shorter (`lesson-goal` 501px → 491px, bridge rows 95px → 87px). **An English-only class was written first and thrown away**; the sizes were wrong for both languages, so there is no language branch and nothing for a Korean deck to opt into. Ceiling worth knowing: 26px carries six full-width glyphs per bridge column |
| `.phone.paged > .pg-on { padding-bottom: 88px }` — drop `.section` from the selector | **written 2026-08-19 · publish pending** | Only `.section` had pager clearance, but `min-height: calc(100vh - 118px)` hid that: at a tall window every page was propped to fit and the usual check passed. Measured with the prop removed, **46 of 59 decks** put their `lesson-goal` tail under the bar at an 800px viewport (needed 852–905px). The shared selector now fixes every deck and every future one; no deck-local workaround is allowed |
| `.bt-word` | local only, **not in v1.3.0** | groups a dictionary form as one word in a rule diagram |
| `.pattern-meaning` / `.meaning-kicker` | **shipped — in v1.7.0** | the 뜻과 쓰임 box every `pN-teach` opens with. It renders correctly in class now; the row is kept so nobody re-adds it to this queue |
| `.anchor` / `.anchor-ja` / `.anchor-ko` | local only, **not in v1.7.0** — **now the top of this queue** | the anchor slot inside `.pattern-meaning`: the Japanese equivalent with its Hangul reading under it, replacing the sentence that used to quote the Japanese inline. Unstyled these are two bare inline spans, so the Japanese, the reading and the Korean sentence run together on one line — worse than the old prose, not merely plainer. **Publish the runtime before rolling the slot out beyond 과 92**, or the decks that adopt it read as one mangled run in class |
...
## 칩은 4개가 천장이자 기본값, 질문은 빈칸 하나에 하나

두 규칙 모두 트랙 전체를 한 번 훑고 나서 고친 것이라, 새로 쓰는 사람은 처음부터 지킨다.
자세한 근거는 `ux-philosophy.md` 와 이 트랙의 `lesson-blueprint.md` 에 있다.

**reorder 칩.** 한 문장 = 의미 덩어리 4개. 문장이 정말 3덩어리면 3개로 두되(억지로 늘리지
않는다), **하나인 것을 쪼개서 4개를 만들지 않고**(`일본 사람`, 조사와 명사, 어미와 어간,
`수 있어요` · `고 싶어요` 같은 그 과의 패턴), **이미 있는 네 번째 덩어리를 옆에 붙여 두지도
않는다**. 후자가 이 트랙이 실제로 저지른 쪽이었다 — 348문장 중 51문장이 부사·시간·목적어를
이웃 칩에 녹여 3개로 만들어 두고 있었다. 같은 페이지 안에서는 자르는 기준이 같아야 한다.

**질문.** 빈칸 하나를 받는 대사는 질문 하나다. 두 개는 뒤엣것이 앞엣것을 좁힐 때만
(양자택일 `추워요? 더워요?`, 또는 본 질문을 여는 예/아니오). 세 개는 안 되고, 서로 다른
