...
튜터 대본은 정확한 작업 지시의 목록이 아니라 학습자와 함께 수업을 이끄는 자연스러운
대화입니다. 같은 뜻이라면 짧고 사무적인 명령보다 아래 말투를 우선합니다.

- `오늘은`, `먼저`, `이번엔`, `지금까지`로 지금 하는 활동과 앞뒤 흐름을 밝혀 줍니다.
- **학습자 지시의 어미는 취향이 아니라 활동이 정합니다.** 화면을 누르고 고르고 적는
  조용한 동작은 명령형 `눌러 보세요`, 소리 내어 하는 활동은 권유형 `읽어 볼까요?`입니다.
  같은 동사가 두 활동에 다 쓰입니다 — `일본어를 보고 빈칸을 채워 보세요`는 손으로 쓰는
  활동이라 명령형이고, `나 역할로 대답해 볼까요?`는 소리 내어 하는 활동이라 권유형입니다.
  **어미를 고치면 일본어 짝도 같이 고칩니다**: 명령형은 `てください` / `てみましょう`,
  권유형은 `てみましょうか`. 한쪽만 고치면 두 언어가 다른 말을 합니다.
  강제되는 동사 표는 [`tools/authoring/kr/tone-standard.json`](../../../tools/authoring/kr/tone-standard.json)
  에 있고 `check_tone.py` 가 검사합니다. 아직 갈린 동사는 `mixed` 에 두고 보고만 합니다.
- 튜터가 시범으로 읽어 주는 자리는 평서형 `제가 먼저 읽을게요`입니다. `읽어 볼게요`는
  튜터가 스스로 시도해 본다는 뜻이 되어 시범과 어긋납니다. 한국어 두 형태 모두 일본어는
  `私が読みます`라서, 이 자리를 고칠 때 일본어는 그대로 둡니다.
- 화면에 이미 제시된 모델 문장을 읽는 활동은 튜터가 네 문장을 먼저 읽고 학습자가 뒤따르는
  2회전 활동으로 만들지 않습니다. `이번엔 네 문장을 한 줄씩 소리 내어 읽어 볼까요?`처럼
  학습자가 바로 읽게 합니다. 발음 시범이 필요한 한글·발음 활동과, 튜터와 학습자가 서로
  다른 역할을 맡아 주고받는 대화 활동은 예외입니다.
- 이미 배운 내용은 `이미 알고 있는 글자죠?`, `이렇게만 기억해 주시면 돼요`처럼 안심과
  성취감을 주되, 학습자가 실제로 아직 모르는 내용을 안다고 단정하지 않습니다.
- `막히면`, `읽고`, `고른 뒤`처럼 주체가 헷갈릴 수 있는 지시는 `학생이 막히면`,
  `학생이 읽고 나면`처럼 누가 무엇을 하는지 분명히 씁니다.
- 화면에 보이는 준비 상태를 설명하는 대신, 왜 하는 활동인지와 다음 행동을 말합니다.
- 자연스러운 문장이 조금 길어지더라도 튜터가 실제 수업에서 그대로 말할 수 있는 문장을
  우선합니다. 다만 활동을 방해하는 장황한 설명이나 화면에 이미 보이는 정보는 늘리지 않습니다.
- 한국어를 이 말투로 고치면 일본어도 같은 주체, 초대의 강도, 안심과 연결 관계를 갖도록
  함께 고칩니다.

새 수업은 이 기준으로 처음부터 쓰고, 기존 수업을 교정할 때도 한 문장만 고친 뒤 멈추지
말고 같은 기능의 페이지를 과정 전체에서 찾아 반복되는 문제를 함께 고칩니다.

### 한글 트랙에서 합의한 반복 문구

원어민 검토에서 같은 지적이 다시 나오지 않도록, 한글 트랙의 뜻이 같은 반복 페이지는
아래 문구로 통일합니다. 위의 말투 원칙을 한글 활동에 적용한 확정 문구입니다.

- 실제 항목을 읽는 마무리 활동은 `단어 읽기`라고 부르고, `지금까지 배운 글자로 된
  단어예요. 소리 내어 읽어 볼까요?` / `これまでに習った文字でできた単語です。声に出して
  読んでみましょうか。`라고 말합니다.
- 소리 구별 페이지의 기본 지시는 `제가 읽을게요. 잘 듣고 맞는 쪽을 눌러 보세요.` /
  `私が読みます。よく聞いて、正しいほうをタップしてください。`입니다. 듣고 만드는 페이지도
  `잘 듣고` / `よく聞いて`를 함께 씁니다.
- 따라 읽기 페이지의 기본 지시는 `제가 먼저 읽을게요. 잘 듣고 따라 읽어 보세요.` /
  `私が先に読みます。よく聞いて、まねして読んでみましょう。`입니다.
- 티칭 모드가 정답을 표시한 뒤의 튜터 메모는 무엇을 읽는지 바로 알 수 있도록
  `정답으로 표시된 쪽을…`이라고 씁니다.
- 화면에서 이미 보이는 `색이 없어요` 같은 준비 상태는 말하지 않고 학습자의 다음 행동만
  말합니다. 페이지가 제시한 항목은 `위에 쓰여 있는`이라고 자연스럽게 지칭합니다.
- 튜터 전용 회복 지시는 `학생이 막히면…`처럼 주체를 밝힙니다. 학습자 대사나 문법 예문의
  `길이 막히면`처럼 뜻이 분명한 문장에는 이 규칙을 적용하지 않습니다.
- 수업의 흐름은 `오늘은`, `먼저`, `이번엔`, `지금까지`로 연결하고, `한 번 읽어 볼까요?`,
  `이미 알고 있는 글자죠?`처럼 초대·안심의 말투를 씁니다. 튜터 모델링은 새 소리나 대화
  상대역처럼 학습상 필요한 경우에만 씁니다.
  같은 기능의 다른 페이지도 이 말투를 기준으로 고칩니다.
