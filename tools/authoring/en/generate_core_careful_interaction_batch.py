...
LESSONS = {
    ...
    63: dict(
        ...
        p2=rows(
            ...
            ("{t}I tend to avoid{/t} rush hour.", "私は{t}ラッシュアワーを避けることが多いです{/t}。", "I tend to|avoid|rush hour."),
            ("{t}I tend to avoid{/t} noisy restaurants.", "私は{t}騒がしいレストランを避けることが多いです{/t}。", "I tend to|avoid|noisy restaurants."),
            ("{t}I tend to avoid{/t} anything too expensive.", "私は{t}高すぎるものを避けることが多いです{/t}。", "I tend to|avoid|anything too expensive."),
        ),
    ...
    65: dict(
        ...
        p1=rows(
            ...
            ("My bags {t}were stolen{/t}.", "かばんを{t}盗まれました{/t}。", "My bags|were stolen."),
            ("My phone {t}was stolen{/t}.", "携帯を{t}盗まれました{/t}。", "My phone|was stolen."),
            ("My documents {t}were stolen{/t}.", "書類を{t}盗まれました{/t}。", "My documents|were stolen."),
        ),
        p2=rows(
...
VOCAB = {
    60: dict(new="window|窓; close|閉める; open|開ける; mind|気にする", recycled="", assumed="door|ドア; check|確認する; file|ファイル; send|送る; email|メール; thanks|ありがとう; thank you|ありがとう; no rush|急ぎではない", receptive="favor|お願い; impose|負担をかける; urgent|緊急の"),
    ...
    62: dict(new="meeting room|会議室; next train|次の電車; leaves|出発する; know|分かる; tell|教える", recycled="station|駅|CORE-7; bank|銀行|CORE-7; café|カフェ|CORE-7; shop|店|CORE-10; meeting|会議|CORE-30; upstairs|2階|CORE-56", assumed="opens|開く; starts|始まる; closes|閉まる; blocks away|ブロック先; departure time|出発時刻; help|助け; thank you|ありがとう; great|分かった; perfect|よかった; go|行く", receptive="sorry to bother you|お邪魔してすみません; by any chance|もしかして; platform|ホーム"),
    63: dict(new="generally|たいてい; tea|紅茶; noisy|騒がしい; avoid|避ける; tend|傾向がある; rush hour|ラッシュアワー", recycled="prefer|〜のほうが好き|CORE-42; coffee|コーヒー|CORE-9; café|カフェ|CORE-7; restaurant|レストラン|CORE-10; crowded|混んでいる|CORE-44; expensive|高い|CORE-19", assumed="train|電車; buses|バス; mornings|朝; evenings|夜; places|場所; kinds|種類; strong preference|強い好み; great|よかった; go in|中に入る; take it|それに乗る", receptive="in general|一般に; as a rule|概して"),
    ...
    65: dict(new="stolen|盗まれた; taken from|から取られた; pocket|ポケット; desk|机", recycled="wallet|財布|CORE-55; ticket|チケット|CORE-9; bag|かばん|CORE-3; please|お願いします|CORE-9", assumed="phone|携帯; car|車; lost|なくした; something important|大切な物; call|電話する; find|見つける; everything|すべて; know|分かる; thank you|ありがとう", receptive="theft|盗難; apparently|どうやら; it turns out|結局分かる"),
    66: dict(new="more time|もっと時間; more money|もっとお金; free month|1か月の休み; abroad|海外; bigger|より広い; anywhere|どこでも", recycled="move|引っ越す|CORE-37", assumed="won|当たった; travel|旅行する; study|勉強する; work at home|家で働く; room|部屋; Osaka|大阪; sounds fun|楽しそう; sounds peaceful|落ち着きそう", receptive="imaginary|想像上の; in a perfect world|理想を言えば; realistically|現実的には"),
    67: dict(new="free time|自由な時間; energy|元気; quieter|より静かな; schedule|予定; wish|願う", recycled="drive|運転する|CORE-14; work from home|在宅勤務をする|CORE-25", assumed="cook|料理する; travel|旅行する; better|より上手に; room|部屋; nothing comes to mind|何も思いつかない; good idea|いい考え; help a lot|とても助かる", receptive="dissatisfaction|不満; if only|さえあれば; oh well|まあ仕方ない"),
...
TRANSLATE_HINTS = {
    ...
    63: (("コーヒー:coffee; 紅茶:tea", "電車:train; バス:bus", "カフェ:café; レストラン:restaurant", "朝:morning; 夜:evening"), ("混んでいる:crowded; 場所:places", "ラッシュアワー:rush hour", "騒がしい:noisy; レストラン:restaurants", "高すぎる:too expensive")),
    ...
    65: (("財布:wallet; 盗む:steal", "かばん:bags; 盗む:steal", "携帯:phone; 盗む:steal", "書類:documents; 盗む:steal"), ("かばん:bag; 取る:take", "ポケット:pocket; 取る:take", "机:desk; 取る:take", "車:car; 取る:take")),
