...
SPECS = {
    ...
    95: semantic_spec(
        ...
        (("Focus one area", "一つの領域に絞る", "Where I'd push back + is + <b>on</b> + 一点", "Not the whole proposal", "提案全体ではない", ("Where I'd push back is <b>on the cost</b>.", "Where I'd push back is <b>on the timing</b>.")), ("Insert the missing factor", "抜けた要素を入れる", "take + 要素 + <b>into account</b>", "Included in the judgment", "判断に含める", ("<b>Take into account</b> the delay before deciding.", "<b>Take into account</b> the risk before deciding."))),
...
VOCAB = {
    ...
    101: dict(new="depends on whether|〜かどうかによる; either way|どちらにしても", recycled="approval|承認|CORE-82; free|空いている|CORE-28; deadline|期限|CORE-82; hear back|返事をもらう|CORE-80; schedule|予定|CORE-67", assumed="update|更新; approve|承認する; room|会議室; price|価格; prepare|準備する; backup plan|予備案; same|同じ; unresolved|未確定の; plan|案; answer|答え; change|変わる; good|よい; great|よい; lose|失う; manageable|対応できる; need|必要である; outcome|結果; send|送る; stay|変わらない; time|時間; today|今日", receptive="up to them|彼ら次第; we'll cross that bridge|その時に考える; in either case|どちらの場合も; checklist|確認表"),
