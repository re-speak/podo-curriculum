...
TOPICS = {
    21: topic(
        ...
        prompts=[
            prompt("A familiar film", "何度も見た映画", "What film have you seen more than twice?", "3回以上見た映画は何ですか？", ["If one comes to mind, when did you last watch it?", "If none does, which film have you come closest to rewatching?"]),
            ...
            prompt("How many times", "見た回数", "About how many times have you seen it?", "その映画をだいたい何回見ましたか？", ["Were those viewings close together?", "Which viewing do you remember best?"]),
            prompt("What you notice now", "今気づくこと", "If you've seen it more than once, what do you notice now that you missed the first time?", "二回以上見たなら、最初は気づかなかったことで、今は何に気づきますか？", ["If you rewatched it, what changed the way you saw it?", "If you saw it once, what might you notice next time?"]),
            prompt("Who you watched with", "一緒に見た人", "Who have you watched it with, if anyone?", "誰かと見たことがあるなら、誰と見ましたか？", ["If someone has watched it with you, how did they react?", "If nobody has, how might another viewer react?"]),
            prompt("What others miss", "伝わりにくい魅力", "What do people not get about this film?", "この映画について、ほかの人に伝わりにくい点は何ですか？", ["If you have heard criticism, which point comes up most?", "If you have not, what might a new viewer misunderstand?"], full="What do other people fail to appreciate about this film?"),
    ...
    22: topic(
        ...
        prompts=[
            prompt("The first enjoyable thing", "最初に楽しめたもの", "What was the first thing in English you actually enjoyed — song, film, game, anything?", "歌や映画、ゲームなど何でもよいので、英語で初めて本当に楽しめたものは何でしたか？", ["If one comes to mind, what do you remember about it?", "If none does, what has come closest to feeling enjoyable?"]),
            ...
            prompt("What brought you here", "今につながったもの", "How did that experience lead to the English you use now?", "その体験は、今使っている英語にどうつながりましたか？", ["Which part of the connection is strongest?", "What might have happened without it?"], full="How did that experience shape your relationship with English now?"),
    ...
    23: topic(
        ...
        prompts=[
            prompt("Your commute", "通勤中の音楽", "What do you listen to while you commute?", "通勤中は何を聴きますか？", ["If you commute, what does it do for the journey?", "If you do not, when do you usually listen instead?"]),
            ...
            prompt("A risky shuffle", "シャッフルの危険", "Is there any song that would embarrass you if it played aloud on shuffle?", "シャッフルで大きな音で流れたら恥ずかしい曲はありますか？", ["If one comes to mind, what setting would make it awkward?", "If none does, what kind of song could still surprise people?"]),
    ...
    24: topic(
        ...
        prompts=[
            prompt("Your standard song", "いつもの一曲", "What do you always sing at karaoke?", "カラオケではいつも何を歌いますか？", ["If you have one, when did it become your regular choice?", "If not, which song would feel safest to try?"]),
            prompt("Your last karaoke", "最近のカラオケ", "When did you last go to karaoke, if ever?", "カラオケに行ったことがあるなら、最後に行ったのはいつですか？", ["If you have been, who were you with?", "If not, what kind of group would make you want to go?"]),
            ...
            prompt("A song for me", "私へのおすすめ", "Ask me about my voice, range, taste, and confidence, then recommend a karaoke song.", "私の声、音域、好み、自信について聞いてから、カラオケの曲をすすめてください。", ["Why does that song fit what I told you?", "What should my backup be?"]),
        ],
    ),
    25: topic(
        ...
        prompts=[
            ...
            prompt("A recent late meal", "最近の夜食", "When did you last eat something late at night, if ever?", "夜遅く食べたことがあるなら、最後はいつですか？", ["If you remember one, what kept you awake?", "If not, what situation might lead you to eat that late?"]),
            ...
            prompt("How often", "頻度", "How often do you eat after 10 p.m.?", "夜10時以降にどのくらい食べますか？", ["If you ever do, what tends to cause it?", "If you never do, what helps you avoid it?"]),
    ...
    26: topic(
        ...
        prompts=[
            ...
            prompt("Have you tried it?", "食べたことは？", "Have you actually tried pineapple on pizza?", "実際にパイナップルのピザを食べたことはありますか？", ["If yes, what was the taste like?", "If not, what do you imagine would be wrong?"]),
            ...
            prompt("Your own combination", "自分の組み合わせ", "What food combination do you like that other people find strange?", "自分は好きでも、ほかの人が変だと思う食べ合わせは何ですか？", ["If one comes to mind, how did you first try it?", "If none does, which unusual pairing would you still consider trying?"]),
            prompt("Someone else's choice", "ほかの人の選択", "What food pairing have you seen someone enjoy but could not watch?", "誰かが喜んで食べていても、自分には見ていられなかった食べ合わせは何ですか？", ["If one comes to mind, what exactly bothered you?", "If none does, which pairing would be hardest to watch?"]),
            prompt("A changed mind", "変わった好み", "Is there a food combination you changed your mind about?", "考えが変わった食べ合わせはありますか？", ["If yes, what caused the change?", "If not, which opinion has stayed strongest?"]),
    ...
    27: topic(
        ...
        prompts=[
            prompt("A place you protect", "教えたくない店", "Is there a restaurant you love but don't want too many people to know about?", "大好きだけれど、あまり多くの人に知られたくない店はありますか？", ["If yes, which restaurant will you use for the discussion?", "If not, which restaurant could you use or imagine?"]),
            prompt("The kind of place", "どんな店？", "What is the real or imagined restaurant like?", "実在する店でも想像の店でもかまいませんが、どんなレストランですか？", ["What kind of food does it serve?", "What makes the atmosphere feel special?"]),
            prompt("Where it is", "場所", "Where is it, or where would it be?", "その店はどこにありますか、または想像の店ならどこにありそうですか？", ["What is the area like?", "How easy would it be to find?"]),
            prompt("Your order", "選びたい注文", "What do you order there, or what would you order?", "そこで何を注文しますか、または想像の店なら何を注文したいですか？", ["Why would that be your first choice?", "What else might be worth trying?"]),
            prompt("How it is found", "見つける方法", "How did you find it, or how might someone discover it?", "その店をどう見つけましたか、または想像の店ならどうやって見つけると思いますか？", ["What would make someone return?", "Would they expect it to be good at first?"]),
            prompt("Who you would take", "連れて行きたい人", "Who have you taken there, or who would you take?", "そこに誰かを連れて行ったことはありますか、または誰を連れて行きたいですか？", ["How did they react, or how might they react?", "Would sharing it change the place for you?"]),
    ...
    28: topic(
        ...
        prompts=[
            prompt("A book to give away", "人に贈った本", "Have you ever bought a book to give away?", "誰かに贈るために本を買ったことはありますか？", ["If yes, did you write a note inside?", "If not, which book would you most like to give someone?"]),
            prompt("A book you received", "もらった本", "Has anyone ever chosen a book for you?", "誰かがあなたのために本を選んでくれたことはありますか？", ["If yes, what did their choice say about you?", "If not, who would understand your taste well enough?"]),
            prompt("The book and person", "本と相手", "Tell me about the book you chose or would choose for someone else.", "誰かのために選んだ本、または選びたい本について話してください。", ["Who is the person?", "What would be the occasion?"]),
            prompt("Did they read it?", "読んでくれた？", "If you gave the book, do you know whether they read it?", "その本を贈ったなら、相手が読んだか知っていますか？", ["If they read it, what did they say?", "If you have not given it, what response would show that your choice worked?"]),
            ...
            prompt("What it did for you", "自分への影響", "What has the book meant to you, if you have read it?", "自分で読んだことがあるなら、その本は自分にとってどんな意味がありましたか？", ["If you read it, which idea stayed with you?", "If not, why do you still trust the recommendation?"]),
    ...
    29: topic(
        ...
        prompts=[
            prompt("A misunderstood hobby", "誤解される趣味", "What do you do that other people think is boring?", "自分はしているけれど、ほかの人が退屈だと思うことは何ですか？", ["If one comes to mind, which one will you use for the discussion?", "If none does, which hobby are you curious about but do not do?"]),
            ...
            prompt("How it begins", "始めるきっかけ", "How did you get into it, or how might a beginner start?", "自分はどう始めましたか、またはしていない趣味なら初心者はどう始めると思いますか？", ["What might make someone continue after the first try?", "Who could help a beginner understand it?"]),
            prompt("Time for it", "使う時間", "How much time do you give it, or think it would need?", "その趣味にどのくらい時間を使いますか、またはしていないならどのくらい必要だと思いますか？", ["How could someone make room for it?", "When might the time feel well spent?"]),
    ...
    30: topic(
        ...
        prompts=[
            prompt("The object", "その物", "Look around your room. What would you not throw away?", "部屋を見回してください。絶対に捨てない物は何ですか？", ["If you are not in your room, what object comes to mind?", "What does it look or feel like?"]),
            prompt("Something nearby", "近くにある物", "What object near you has been with you the longest?", "今近くにある物で、いちばん長く持っているものは何ですか？", ["How often do you use or notice it?", "Where do you usually keep it?"]),
            prompt("How you got it", "手に入れた経緯", "How did you get the object?", "その物をどうやって手に入れましたか？", ["If someone else was involved, what did they add to the story?", "If nobody else was, what made the moment memorable?"]),
            ...
            prompt("Who knows", "大切さを知る人", "Who else, if anyone, knows why it matters to you?", "なぜ大切なのか、ほかに知っている人はいますか？", ["If someone knows, how would they explain it?", "If nobody does, which part would be hardest to explain?"]),
            prompt("One you regret losing", "手放して後悔した物", "Have you ever thrown something away and regretted it?", "物を捨てて後悔したことはありますか？", ["If yes, what made you miss it?", "If not, how do you decide what to keep?"]),
    ...
    31: topic(
        ...
        prompts=[
            prompt("Your main colour", "多い色", "What colour is most of your wardrobe?", "手持ちの服でいちばん多い色は何ですか？", ["If one stands out, why does it work for you?", "If none does, which color do you choose most often?"]),
            ...
            prompt("Owned but unworn", "持っているけれど着ない服", "What, if anything, do you own but almost never wear?", "持っているけれど、ほとんど着ないものはありますか？", ["If one comes to mind, why did you buy or keep it?", "If none does, what helps you avoid unused clothes?"]),
            ...
            prompt("Someone who dresses well", "おしゃれな人", "Who, if anyone, dresses well around you?", "身近に、服の着こなしが上手な人はいますか？", ["If someone comes to mind, what do they understand about clothes?", "If nobody does, what kind of style still looks well put together to you?"]),
    ...
    32: topic(
        ...
        prompts=[
            ...
            prompt("Used today", "今日使った古い物", "Did you use anything old today?", "今日は何か古い物を使いましたか？", ["If yes, what made it useful?", "If not, which old item do you use most often?"]),
    ...
    33: topic(
        ...
        prompts=[
            prompt("Eating to be polite", "礼儀のために食べるもの", "Is there something you eat to be polite?", "礼儀のために食べるものはありますか？", ["If yes, how much do you usually eat?", "If not, what kind of food would be hardest to accept politely?"]),
            prompt("A recent table", "最近の食卓", "When did you last eat something you did not really want, if ever?", "食べたくないものを食べたことがあるなら、最後はいつですか？", ["If you remember one, how did it end up on your plate?", "If not, what situation would make that most likely?"]),
            ...
            prompt("What you would say", "正直に言う？", "Would you say directly that you did not like it?", "好きではないと、はっきり言いますか？", ["If yes, how would you phrase it kindly?", "If not, whose feelings would you protect?"]),
            ...
            prompt("At a dinner", "食事会で言うなら", "Would you say you disliked the food at a dinner?", "食事会で、その料理が好きではなかったと言いますか？", ["What would you say to the host?", "When would honesty need to be more direct?"]),
        ],
    ),
}
...
def build(topic_no: int, variant: str) -> str:
    ...
    pages = [
        f'''    <div class="transition-page" data-page-id="lesson-goal" data-act="Things I like">
      <span class="transition-kicker">THINGS I LIKE</span>
